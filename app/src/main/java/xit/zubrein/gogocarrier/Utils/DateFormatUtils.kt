...
class DateFormatUtils {
    ...
    private val preFormatAllOrder = SimpleDateFormat("dd-MM-yyyy")
    ...
    fun getDateAllOrder(dateStr: String): Array<String> {
        val newFormatYear = SimpleDateFormat("yyyy", Locale.getDefault())
        val newFormatMonth = SimpleDateFormat("MMM", Locale.getDefault())
        val newFormatDay = SimpleDateFormat("dd", Locale.getDefault())
        var year = ""
        var month = ""
        var day = ""
        try {
            year = newFormatYear.format(preFormatAllOrder.parse(dateStr))
            month = newFormatMonth.format(preFormatAllOrder.parse(dateStr))
            day = newFormatDay.format(preFormatAllOrder.parse(dateStr))
        } catch (e: ParseException) {
            e.printStackTrace()
        }
        return arrayOf(year, month, day)
    ...
    fun getDateWithMontheName(dateStr: String): String{
